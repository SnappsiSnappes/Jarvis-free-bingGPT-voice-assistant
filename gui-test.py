...
c = conn.cursor()
c.execute('CREATE TABLE IF NOT EXISTS mytable (id INTEGER PRIMARY KEY, name TEXT, action TEXT, voice TEXT, data TEXT)')
...
def add_item():
    ...
    if action and voice and data:
        c.execute('INSERT INTO mytable (action, name, voice, data) VALUES (?, ?, ?, ?)', (action, name, voice, data))
        conn.commit()
        update_listbox()
...
delete_button = tk.Button(root, text='Удалить', command=delete_item, width=10, height=2,font=('TkDefaultFont', 13))
delete_button.pack()
...
action_menu.config(width=20, height=2,font=('TkDefaultFont', 13))
action_menu.pack()
name_label = tk.Label(root, text='Имя:',font=('TkDefaultFont', 13))
name_label.pack()
name_entry =  tk.Entry(root,width=35,font=('TkDefaultFont', 13))
name_entry.pack()
...
add_button = tk.Button(root, text='Добавить', command=add_item, width=10, height=2,font=('TkDefaultFont', 13))
add_button.pack()
